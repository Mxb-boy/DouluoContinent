...
local UGC_Task_ItemGet_UIBP = { bInitDoOnce = false } 

function UGC_Task_ItemGet_UIBP:Construct()
	self:InitBindEvent();
...
function UGC_Task_ItemGet_UIBP:InitBindEvent()
...
function UGC_Task_ItemGet_UIBP:InitUpItem(Item, Index)
    local ItemID = self.ItemUpList[Index + 1].ItemID;
    local ItemNum = self.ItemUpList[Index + 1].ItemNum;
    Item:InitUI(ItemID, ItemNum);
end

function UGC_Task_ItemGet_UIBP:InitDownItem(Item, Index)
    local ItemID = self.ItemDownList[Index + 1].ItemID;
    local ItemNum = self.ItemDownList[Index + 1].ItemNum;
    Item:InitUI(ItemID, ItemNum);
end

function UGC_Task_ItemGet_UIBP:ConfirmBtnClick()
    self:SetVisibility(ESlateVisibility.Collapsed);
end

-- function UGC_Task_ItemGet_UIBP:Tick(MyGeometry, InDeltaTime)

-- end

function UGC_Task_ItemGet_UIBP:Destruct()
    self.UGC_ReuseList2_Item1.OnAfterNewItem:RemoveAll();
    self.UGC_ReuseList2_Item2.OnAfterNewItem:RemoveAll();
    self.NewButton_Confirm.OnClicked:RemoveAll();
end
...
function UGC_Task_ItemGet_UIBP:InitUI(ItemList)
    if ItemList then
        ...
        if self.WwiseEvent then
            UGCSoundManagerSystem.PlaySound2D(self.WwiseEvent);
        end
    end
end

return UGC_Task_ItemGet_UIBP
