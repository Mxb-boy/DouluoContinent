---@class SignInEvent_ApplyProp_Popup_UIBP_C:UUserWidget
---@field Btn_Increase UButton
---@field Btn_Increase100 UButton
---@field Btn_Reduce UButton
---@field BtnClose UButton
---@field Button_Buy UButton
---@field Common_PopupsBg_Medium_UIBP Common_PopupsBg_Medium_UIBP_C
---@field Common_UIPopupBG Common_UIPopupBG_C
---@field HorizontalBox_Won UHorizontalBox
---@field Image_Bg_Quality UImage
---@field Image_EffectExpression UImage
---@field Image_Icon UImage
---@field Image_Quality UImage
---@field IsOwned UTextBlock
---@field Name1 UTextBlock
---@field NumberInput UHorizontalBox
---@field Text UTextBlock
---@field TextBlock_Count UTextBlock
---@field TextBlock_Describe UTextBlock
---@field Title UTextBlock
---@field WidgetSwitcher_Btn UWidgetSwitcher
---@field WidgetSwitcher_Increase UWidgetSwitcher
---@field WidgetSwitcher_Reduce UWidgetSwitcher
--Edit Below--
local SignInEvent_ApplyProp_Popup_UIBP = { bInitDoOnce = false } 

--[==[ Construct
function SignInEvent_ApplyProp_Popup_UIBP:Construct()
	
end
-- Construct ]==]

-- function SignInEvent_ApplyProp_Popup_UIBP:Tick(MyGeometry, InDeltaTime)

-- end

-- function SignInEvent_ApplyProp_Popup_UIBP:Destruct()

-- end

return SignInEvent_ApplyProp_Popup_UIBP