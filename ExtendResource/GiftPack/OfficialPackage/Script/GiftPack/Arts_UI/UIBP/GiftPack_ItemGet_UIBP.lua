---@class GiftPack_ItemGet_UIBP_C:UUserWidget
...
local GiftPack_ItemGet_UIBP = { bInitDoOnce = false } 

function GiftPack_ItemGet_UIBP:Construct()
	self:InitBindEvent();
end

function GiftPack_ItemGet_UIBP:InitBindEvent()
...
function GiftPack_ItemGet_UIBP:InitUpItem(Item, Index)
    local ItemID = self.ItemUpList[Index + 1].ItemID;
    local ItemNum = self.ItemUpList[Index + 1].ItemNum;
    Item:InitUI(ItemID, ItemNum);
end

function GiftPack_ItemGet_UIBP:InitDownItem(Item, Index)
    local ItemID = self.ItemDownList[Index + 1].ItemID;
    local ItemNum = self.ItemDownList[Index + 1].ItemNum;
    Item:InitUI(ItemID, ItemNum);
end

function GiftPack_ItemGet_UIBP:ConfirmBtnClick()
    self:SetVisibility(ESlateVisibility.Collapsed);
end

-- function GiftPack_ItemGet_UIBP:Tick(MyGeometry, InDeltaTime)

-- end

-- function GiftPack_ItemGet_UIBP:Destruct()
