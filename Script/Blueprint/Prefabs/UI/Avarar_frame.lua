...
--[[----------------------设置头像显示的玩家等级------------------------]]
function Avarar_frame:SetPlayerStateLevel(Level)
    self.PlayerStateLevel = tonumber(Level)
end
...
function Avarar_frame:SetHeadImageByPlayerKey(PlayerKey)
    ...
    self:print(string.format("UID:%s,IconURL:%s,playerlevel:%s", tostring(UID), tostring(IconURL),
        tostring(AccountInfo.PlayerLevel)))
    local PlayerStateLevel = self.PlayerStateLevel
    if PlayerStateLevel == nil then
        local PlayerState = UGCGameSystem.GetLocalPlayerState()
        PlayerStateLevel = PlayerState and PlayerState:GetPlayerLevel() or AccountInfo.PlayerLevel
    end

    self.Avatar:InitView(2, UID, IconURL, AccountInfo.Gender, 0, PlayerStateLevel, false, false)
