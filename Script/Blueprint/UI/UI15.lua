...
function UI15:GetSoulRingGrid()
    return self:GetWidget("WrapBox_0")
end

function UI15:GetSoulRingTableRows()
...
function UI15:GetSoulRingName(ItemID, Row)
...
function UI15:GetItemIDFromDefineID(ItemDefineID)
...
function UI15:GetSoulRingsFromBackpackV2(PlayerPawn)
    local Result = {}
...
function UI15:GetSoulRingsFromLegacyBackpack(PlayerPawn)
    local Result = {}
...
function UI15:GetSoulRingsInBackpack(PlayerPawn)
...
function UI15:ParseSoulRingInventorySnapshot(Snapshot)
    local Result = {}
...
function UI15:ApplySoulRingInventorySnapshot(Snapshot)
    self:RefreshSoulRingList(self:ParseSoulRingInventorySnapshot(Snapshot))
end

function UI15:GetSoulRingIconPath(ItemID, Row)
...
function UI15:RefreshSoulRingList(SoulRingSnapshot)
    local Grid = self:GetSoulRingGrid()
    if Grid == nil then
        ugcprint("[UI15:RefreshSoulRingList] WrapBox_0 is nil")
        return
    end

    local PlayerPawn = UGCGameSystem.GetLocalPlayerPawn()
    if PlayerPawn == nil then
        ugcprint("[UI15:RefreshSoulRingList] local player pawn is nil")
        return
    end
    ...
    if CellClass == nil then
        ugcprint("[UI15:RefreshSoulRingList] wq1 class load failed: " .. CellPath)
        return
    end
    ...
    for _, ItemID in ipairs(SOUL_RING_ITEM_IDS) do
        ...
        if BackpackData ~= nil then
            ...
            if Cell ~= nil then
                ...
                ugcprint("[UI15:RefreshSoulRingList] cell item=" .. tostring(ItemID) ..
    ...
    ugcprint("[UI15:RefreshSoulRingList] created=" .. tostring(CreatedCount) ..
        " children=" .. tostring(ChildCount))
end

function UI15:SelectSoulRing(SelectedCell)
