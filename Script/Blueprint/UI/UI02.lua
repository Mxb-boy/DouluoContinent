...
function UI02:LuaInit()
    ...
    self.Button_92.OnClicked:Add(self.Button_150_OnClicked, self)
    self.Button_95.OnClicked:Add(self.Button_144_OnClicked, self)
...
function UI02:SetTowerOutBoxImageVisible(bVisible)
    self.Image_43:SetVisibility(bVisible and ESlateVisibility.HitTestInvisible or ESlateVisibility.Collapsed)
end
...
function UI02:Button_158_OnClicked()
    ...
    if UI14Class == nil then
        ugcprint("[UI02:Button_158_OnClicked] UI14 class load failed: " .. UI14Path)
        return
    end

    self.UI14Instance = UserWidget.NewWidgetObjectBP(PlayerController, UI14Class)
    if self.UI14Instance == nil then
        ugcprint("[UI02:Button_158_OnClicked] UI14 create failed")
        return
    end
