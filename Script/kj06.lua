...
function kj06:ApplyButtonEffect(Button)
...
function kj06:Btn_Buy_OnClicked()
...
function kj06:PurchaseGiftPack()
    if self:HasPurchasedGiftPack() then
        self:RefreshBuyButtonState()
        return false
    end
    ...
    if PlayerController == nil or ProductID == nil then
        ugcprint("[kj06:PurchaseGiftPack] gift product mapping is not configured")
        return false
    end
...
function kj06:GetShopProductID(ItemID)
...
function kj06:EnsureShopPurchaseCallbacks()
...
function kj06:RemoveShopPurchaseCallback()
...
function kj06:OnGiftPackPurchaseConfirm(Value)
...
function kj06:OnKJ04BuyProductResult(Result)
...
function kj06:GrantRewardsToBackpack()
    local PlayerController = GameplayStatics.GetPlayerController(self, 0)
...
function kj06:HasPurchasedGiftPack()
...
function kj06:HasReachedShopPurchaseLimit()
    local ProductID = tonumber(KJ04_GIFT_PRODUCT.ProductID)
...
function kj06:GetLocalPlayerState()
    local PlayerController = GameplayStatics.GetPlayerController(self, 0)
    return PlayerController and PlayerController.PlayerState or nil
end

function kj06:SetGiftPackPurchased(value)
...
function kj06:RefreshBuyButtonState()
    if self.Btn_Buy == nil then
        return
    end
...
-- function kj06:Tick(MyGeometry, InDeltaTime)

-- end

function kj06:Destruct()
    self:RemoveShopPurchaseCallback()
end
